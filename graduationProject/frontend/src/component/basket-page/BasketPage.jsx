import React from "react";
import "./styles.css";
import pro from "../../assets/14pro.png";

const BasketPage = () => {
  return (
    <div>
      <div class="cart_section">
        <div class="container-fluid">
          <div class="row">
            <div class="col-lg-10 offset-lg-1">
              <div class="cart_container">
                <div class="cart_title fs-4 d-flex align-items-baseline">
                  <p className="fw-bold d-flex">Sepet</p>{" "}
                  <small className="text-muted fs-6 d-flex ms-2">
                    {" "}
                    (Sepetinizdeki ürün sayısı: 1){" "}
                  </small>
                </div>
                <div class="cart_items">
                  <ul class="cart_list">
                    <li class="cart_item clearfix">
                      <div class="cart_item_image">
                        <img src="https://i.imgur.com/qqBRWD5.jpg" alt="" />
                      </div>
                      <div class="cart_item_info d-flex flex-md-row flex-column justify-content-between">
                        <div class="cart_item_name cart_info_col">
                          <div class="cart_item_title">Marka / Model</div>
                          <div class="cart_item_text">Samsung C7 Pro</div>
                        </div>
                        <div class="cart_item_color cart_info_col">
                          <div class="cart_item_title">Renk</div>
                          <div class="cart_item_text">
                            <span style={{ backgroundColor: "#999999" }}></span>
                            Silver
                          </div>
                        </div>
                        <div class="cart_item_quantity cart_info_col">
                          <div class="cart_item_title">Miktar</div>
                          <div class="cart_item_text">1</div>
                        </div>
                        <div class="cart_item_price cart_info_col">
                          <div class="cart_item_title">Fiyat</div>
                          <div class="cart_item_text">₺22000</div>
                        </div>
                        <div class="cart_item_total cart_info_col">
                          <div class="cart_item_title">Toplam</div>
                          <div class="cart_item_text">₺22000</div>
                        </div>
                      </div>
                    </li>
                  </ul>
                </div>
                <div class="order_total">
                  <div class="order_total_content text-md-right">
                    <div class="order_total_title">Toplam Fiyat:</div>
                    <div class="order_total_amount">₺22000</div>
                  </div>
                </div>
                <div class="cart_buttons">
                  {" "}
                  <button type="button" class="button cart_button_clear">
                    Ürünlere Dön
                  </button>{" "}
                  <button type="button" class="button cart_button_checkout">
                    Sepeti Onayla
                  </button>{" "}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default BasketPage;
